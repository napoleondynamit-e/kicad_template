...
4. Use the project-local `kicad-happy` skills for review-readable artifacts and higher-level inspection; do not treat them as a replacement for ERC, DRC, calculations, or datasheet checks.
5. Use `.tools/bin/datasheet` and manufacturer documentation to verify component claims and imported libraries.
6. Report findings first, ordered by severity, with file and location references. Include missing tests and residual risks.

Do not approve unvalidated CAD assets. Do not modify KiCad files during a
